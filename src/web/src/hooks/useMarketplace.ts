...
export const useMarketplace = (initialFilters?: Partial<MarketplaceFilters>) => {
  ...
  const getProductByIdSecure = useCallback(async (productId: string) => {
    try {
  ...
  const purchaseProductSecure = useCallback(async (
    productId: string,
  ...
  return {
    ...
    actions: {
      updateFilters,
      getProductById: getProductByIdSecure,
      purchaseProduct: purchaseProductSecure,
